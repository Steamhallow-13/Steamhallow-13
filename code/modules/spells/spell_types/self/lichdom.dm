...
/datum/action/cooldown/spell/lichdom/cast(mob/living/cast_on)
	...
	if(ishuman(cast_on))
		...
		human_cast_on.equip_to_slot_or_del(new /obj/item/clothing/suit/wizrobe/black(human_cast_on), ITEM_SLOT_O_TORSOWEAR)
		human_cast_on.equip_to_slot_or_del(new /obj/item/clothing/head/wizard/black(human_cast_on), ITEM_SLOT_HEAD)
		human_cast_on.equip_to_slot_or_del(new /obj/item/clothing/under/color/black(human_cast_on), ITEM_SLOT_I_TORSOWEAR)
