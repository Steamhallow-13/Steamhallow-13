...
/datum/component/profound_fisher/Initialize(our_rod)
	...
	if(!isgloves)
		...
		if(istype(wearer) && wearer.get_item_by_slot(ITEM_SLOT_L_HAND) == gloves || wearer.get_item_by_slot(ITEM_SLOT_R_HAND) == gloves)
...
/datum/component/profound_fisher/proc/on_equip(obj/item/source, atom/equipper, slot)
	SIGNAL_HANDLER
	if(slot != ITEM_SLOT_L_HAND || ITEM_SLOT_R_HAND)
		return
